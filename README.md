# Winraycast
This is a refactoring of 1990s ray-cast 3d engine written in C++ running on Windows.
It is not based on OpenGL or DirectX or other 3D library. 
Green field approach used to demostrate the ray-cast algorithm and related to an article published on Computer Programming magazine by Infomedia.

## Screenshots

![Screen Shot](https://github.com/eantcal/Winraycast/blob/master/ScreenShots/ScreenshotCollage.png)

![Screen Shot](https://github.com/eantcal/Winraycast/blob/master/ScreenShots/ScreenShot2.png)

![Screen Shot](https://github.com/eantcal/Winraycast/blob/master/ScreenShots/ScreenShot3.png)

![Screen Shot](https://github.com/eantcal/Winraycast/blob/master/ScreenShots/ScreenShot1.png)

## More information at

[Article on raycast](https://drive.google.com/file/d/1kJ579jaSHmN326CgkOOjtrbpTwqL_gpF/view)
